...
public class SpringReviewDAO {
	...
	// 리뷰 수정
	private static final String sql_UPDATE="UPDATE review content=?, rdate=sysdate  WHERE renum=?";
	...
	public boolean insert(ReviewVO vo) {
		boolean flag = false;
		jdbcTemplate.update(sql_INSERT, vo.getHotelNum(), vo.getMemNum(), vo.getRoomNum(), vo.getContent(), vo.getWriter());
		flag=true;
		return flag;
	}
	// 리뷰 삭제
	...
	public boolean update(ReviewVO vo) {
		boolean flag = false;
		jdbcTemplate.update(sql_UPDATE, vo.getContent(), vo.getReNum());
		flag=true;
		return flag;
	}
